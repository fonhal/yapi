...
class ProjectMessage extends Component {
  ...
  showConfirm = () => {
    ...
    confirm({
      ...
      content: ( <
        ...
        <
        Alert message = "警告：此操作非常危险,会删除该项目下面所有接口，并且无法恢复!"
        type = "warning"
        banner /
        >
        <
  ...
  render() {
      ...
      return ( <
          ...
          className = "setting-intro" >
          <
          h2 className = "ui-title" > {
            (currGroup.group_name || '') + ' / ' + (projectMsg.name || '')
          } <
          /h2> { /* <p className="ui-desc">{projectMsg.desc}</p> */ } <
          /Col> <
          /Row> <
            ...
            getFieldDecorator('desc', {
              ...
              label = "权限" > {
                getFieldDecorator('project_type', {
                  ...
                  /Radio> <
                  br / > {
                    projectMsg.role === 'admin' && < Radio value = "public"
                    className = "radio" >
                    <
                ...
                { /* 只有组长和管理员有权限删除项目 */ } {
                  projectMsg.role === 'owner' || projectMsg.role === 'admin' ? ( <
                    ...
                    /h2> <
                    Button onClick = {
                      this.toggleDangerOptions
                    } >
                    查 看 < Icon type = {
                      this.state.showDangerOptions ? 'up' : 'down'
                    }
                    /> <
                    /Button> <
                    /div> {
                      this.state.showDangerOptions ? ( <
                        ...
                        /div> <
                        Button type = "danger"
                        ghost className = "card-danger-btn"
                        onClick = {
                          this.showConfirm
                        } >
                        删除 <
